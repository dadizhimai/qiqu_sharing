...
jQuery.fn.extend({
    uploadViewPreview: function (opts) {
        $(this).each(function(){
             var _self = this,
                _this = $(this);
            opts = jQuery.extend({
            ...
            _this.change(function () {
                if (this.value) {
                    if (!RegExp("\.(" + opts.ImgType.join("|") + ")$", "i").test(this.value.toLowerCase())) {
                        alert("选择文件错误,视频类型必须是" + opts.ImgType.join("，") + "中的一种");
                        this.value = "";
                        return false
                    }

                     // 查看视频大小(mb)
                     // var fileObj = document.getElementById('videoUp').files[0];
                    var fileObj = this.value.files[0];
                    alert('>>>')
